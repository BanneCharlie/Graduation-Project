 ...
 *
 */
export function abScrollBar(_opts) {
	/* fn */
	var __fns__ = {
		/* extend */
		...
		barEvent: function () {
			_bar.mousedown(function (__e) {
				...
				document.onmousemove = function (__e2) {
					...
					/* 阻止冒泡 */
					if (__e2.preventDefault) {
						__e2.preventDefault();
					}
					if (__e2.stopPropagation) {
		...
		init: function () {
			...
			_contentBox.append(
				'<div id="' +
					...
					'" class="' +
					__barStyle[0] +
					'" style="position:absolute;z-index:2;overflow:hidden;display:none;' +
					__barStyle[2] +
					'"><em class="' +
					__barStyle[1] +
					'" style="position:relative;left:0px;top:0px;width:100%;height:100%;overflow:hidden;border-radius:5px;display:block;' +
					__barStyle[3] +
					'"></em></div>'
			);
