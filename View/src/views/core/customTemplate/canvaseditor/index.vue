...
import { getById } from '@/api/customTemplate/reportTemplate'
...
export default {
  ...
  mounted() {
    this.getDetailTemplateContent(this.genericContentId);
  },
  methods: {
    getDetailTemplateContent(id){
      if (this.mark === 'template') {
        getById(id).then(response => {
          let jsonString = response.data.templateGenericContent;
          if (jsonString !== null && jsonString !== '' && jsonString.includes('header')){
            let templateGenericContent = JSON.parse(jsonString);
            this.genericContent = templateGenericContent.data;
          }else{
            this.genericContent = {
              header:[
                {
          ...
          this.instance = Init(this.genericContent);

          if (this.single !== null) {
            this.instance.instance.command.executeMode(EditorMode.READONLY);
          }

        });
      }else{
        getReportInstace(id).then(response =>{
...
.container {
  ...
  width: 100%; /* 设置父元素的宽度 */
  height: calc(100vh - 160px);/* 设置父元素的高度 */
